...
function configure {
	autoreconf --install
	CC=icx CFLAGS="$(echo "$CFLAGS" | sed 's/-\(feliminate-unused-debug-types\|ffat-lto-objects\|fno-semantic-interposition\|ftree-loop-distribute-patterns\|mrelax-cmpxchg-loop\)//g') -fPIC -fcf-protection -flto=thin -fstack-clash-protection -fstack-protector-strong -ipo -march=x86-64-v3 -mtune=haswell $1" LDFLAGS='-Wl,--as-needed -fuse-ld=lld -rtlib=compiler-rt' ./configure --prefix=/opt/musical-octo-fortnight/usr
}
