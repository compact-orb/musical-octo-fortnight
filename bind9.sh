...
function configure {
	autoreconf --install
	CC=icx CFLAGS="-O3 -fcf-protection -flto=thin -fstack-clash-protection -fstack-protector-strong -ipo -march=x86-64-v3 -mtune=haswell -pipe $1" CPPFLAGS='-D_FORTIFY_SOURCE=3' LDFLAGS='-Wl,--as-needed -Wl,-z,now -Wl,-z,relro -fuse-ld=lld -rtlib=compiler-rt' ./configure --prefix=/opt/musical-octo-fortnight/usr
}
