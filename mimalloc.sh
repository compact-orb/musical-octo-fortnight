...
CC=icx CFLAGS="$(echo "$CFLAGS" | sed 's/-\(Wl,-sort-common\|Wl,-z,relro\|Wl,-z,now\|feliminate-unused-debug-types\|ffat-lto-objects\|ftree-loop-distribute-patterns\|mrelax-cmpxchg-loop\)//g') -fcf-protection -flto -fpic -fpie -fstack-clash-protection -fstack-protector-strong -ipo -march=x86-64-v3 -mtune=haswell" CXX=icpx CXXFLAGS="$(echo "$CXXFLAGS" | sed 's/-\(Wl,-sort-common\|Wl,-z,relro\|Wl,-z,now\|Wl,--enable-new-dtags\|feliminate-unused-debug-types\|ffat-lto-objects\|ftree-loop-distribute-patterns\|mrelax-cmpxchg-loop\)//g') -D_GLIBCXX_ASSERTIONS -fcf-protection -flto -fpic -fpie -fstack-clash-protection -fstack-protector-strong -ipo -march=x86-64-v3 -mtune=haswell" LDFLAGS='-Wl,--as-needed -rtlib=compiler-rt' cmake --install-prefix /opt/musical-octo-fortnight/usr -DCMAKE_BUILD_TYPE=Release -DMI_SECURE=ON .
